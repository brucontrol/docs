...
The Plugin Store lets you browse, install, and update element template plugins. Open it at **`/plugin-store`**.

## Browse

The store lists plugins from the BruControl registry (brucontrol-releases). It has two tabs:

- **Element Templates** — Element templates you can install or update. Each appears in the registry with metadata (name, version, badges, tags).
...
When a newer version of an installed element template plugin is available, an **Update** button appears. Click it to upgrade to the latest version.
